...
class MITBooster
{
    ...
    {
        return DB::table(config('mixtra.USER_TABLE'))->where('id', Session::get('admin_id'))->first();
    }
    ...
    public static function mySkin()
    {
        $skin_color = Session::get('theme_color');
        $skin_color = str_replace('skin-', '', $skin_color);
        return $skin_color;
    }
    ...
    public static function myPrivilege()
    {
        ...
        if ($roles) {
            foreach ($roles as $role) {
                if ($role->path == MITBooster::getModulePath()) {
                    return $role;
                }
            }
        }
    ...
    public static function getCurrentModule()
    {
        ...
        if (Cache::has('modules_'.$modulepath)) {
            ...
            $module = DB::table('mit_modules')->where('path', self::getModulePath())->first();
    ...
    {
        $menu = DB::table('mit_menus')->whereRaw("mit_menus.id IN (select id_mit_menus from mit_menus_privileges where id_mit_privileges = '".self::myPrivilegeId()."')")->where('is_dashboard', 1)->where('is_active', 1)->first();
    ...
    {
        $menu_active = DB::table('mit_menus')->whereRaw("mit_menus.id IN (select mit_menus_id from mit_menus_privileges where mit_privileges_id = '".self::myPrivilegeId()."')")->where('parent_id', 0)->where('is_active', 1)->where('is_dashboard', 0)->orderby('sorting', 'asc')->select('mit_menus.*')->get();
        ...
        foreach ($menu_active as &$menu) {
            ...
            $child = DB::table('mit_menus')->whereRaw("mit_menus.id IN (select mit_menus_id from mit_menus_privileges where mit_privileges_id = '".self::myPrivilegeId()."')")->where('is_dashboard', 0)->where('is_active', 1)->where('parent_id', $menu->id)->select('mit_menus.*')->orderby('sorting', 'asc')->get();
            if (count($child)) {
    ...
    {
        echo "swal({   
				title: \"".trans('mixtra.delete_title_confirm')."\",   
				text: \"".trans('mixtra.delete_description_confirm')."\",   
				type: \"warning\",   
				showCancelButton: true,   
				confirmButtonColor: \"#ff0000\",
				confirmButtonText: \"".trans('mixtra.confirmation_yes')."\",  
				cancelButtonText: \"".trans('mixtra.confirmation_no')."\",
				closeOnConfirm: false, }).then(function(data){
    ...
    public static function sendEmailQueue($queue)
    {
        ...
        \Mail::send("mixtra::emails.blank", ['content' => $html], function ($message) use (
            $html,
            $to,
            $subject,
            $from_email,
    ...
    public static function sendEmail($config = [])
    {
        ...
        $template = MITBooster::first('mit_email_templates', ['slug' => $template]);
        ...
        if ($config['send_at'] != null) {
            ...
            $a['email_from_email'] = $template->from_email ?: MITBooster::getSetting('email_sender');
            $a['email_from_name'] = $template->from_name ?: MITBooster::getSetting('appname');
            $a['email_cc_email'] = $template->cc_email;
            $a['email_subject'] = $subject;
            $a['email_content'] = $html;
            $a['email_attachments'] = serialize($attachments);
            $a['is_sent'] = 0;
            DB::table('mit_email_queues')->insert($a);

            return true;
        }

        \Mail::send("mixtra::emails.blank", ['content' => $html], function ($message) use ($to, $subject, $template, $attachments) {
            $message->priority(1);
            $message->to($to);

            if ($template->from_email) {
                $from_name = ($template->from_name) ?: MITBooster::getSetting('appname');
...
//         }
//         $table = MITBooster::parseSqlTable($table);
...
//     }
    ...
    {
        $parent_table = MITBooster::parseSqlTable($parent_table)['table'];
        $child_table = MITBooster::parseSqlTable($child_table)['table'];
    ...
    {
        $tables = [];
        $multiple_db = config('mixtra.MULTIPLE_DATABASE_MODULE');
        $multiple_db = ($multiple_db) ? $multiple_db : [];
        $db_database = config('mixtra.MAIN_DB_DATABASE');

        if ($multiple_db) {
            try {
                $multiple_db[] = config('mixtra.MAIN_DB_DATABASE');
    ...
    public static function authAPI()
    {
        ...
        $allowedUserAgent = config('mixtra.API_USER_AGENT_ALLOWED');
        ...
        if (self::getSetting('api_debug_mode') == 'false') {
            ...
            $keys = DB::table('mit_apikey')->where('status', 'active')->pluck('screetkey');
            $server_token = [];
            $server_token_screet = [];
            foreach ($keys as $key) {
            ...
            DB::table('mit_apikey')->where('screetkey', $server_screet)->increment('hit');
    ...
    public static function sendNotification($config = [])
    {
        ...
        $to = $config['to'];
        $mit_users_id = $config['mit_users_id'];
        $mit_users_id = ($mit_users_id) ?: [MITBooster::myId()];
        foreach ($mit_users_id as $id) {
            $a = [];
            $a['created_at'] = date('Y-m-d H:i:s');
            $a['mit_users_id'] = $id;
            $a['content'] = $content;
            $a['is_read'] = 0;
            $a['url'] = $to;
            DB::table('mit_notifications')->insert($a);
        }
    ...
    public static function sendFCM($regID = [], $data)
    {
        ...
        $apikey = MITBooster::getSetting('google_fcm_key');
    ...
    {
        $name_col_candidate = config('mixtra.NAME_FIELDS_CANDIDATE');
    ...
    public static function generateAPI($controller_name, $table_name, $permalink, $method_type = 'post')
    {
		...
		class Api'.$controller_name.'Controller extends \mixtra\controllers\ApiController {
    ...
    public static function generateController($table, $name = null)
    {
        ...
        $image_candidate = explode(',', config('mixtra.IMAGE_FIELDS_CANDIDATE'));
        $password_candidate = explode(',', config('mixtra.PASSWORD_FIELDS_CANDIDATE'));
        $phone_candidate = explode(',', config('mixtra.PHONE_FIELDS_CANDIDATE'));
        $email_candidate = explode(',', config('mixtra.EMAIL_FIELDS_CANDIDATE'));
        $name_candidate = explode(',', config('mixtra.NAME_FIELDS_CANDIDATE'));
        $url_candidate = explode(',', config("mixtra.URL_FIELDS_CANDIDATE"));
        ...
        $coloms = MITBooster::getTableColumns($table);
        $name_col = MITBooster::getNameTable($coloms);
        $pk = MIT::pk($table);
...
<?php namespace App\Http\Controllers;
	...
	class Admin'.$controllername.' extends \mixtra\controllers\MITController {
	...
	';
        ...
        foreach ($coloms_col as $c) {
            ...
            if (substr($field, 0, 3) == 'id_') {
                ...
                $joincols = MITBooster::getTableColumns($jointable);
                $joinname = MITBooster::getNameTable($joincols);
        ...
        foreach ($coloms as $c) {
            ...
            $typedata = MITBooster::getFieldType($table, $field);
            ...
            if (in_array($field, $password_candidate)) {
                ...
                $attribute['help'] = trans("mixtra.text_default_help_password");
            }

            if (in_array($field, $image_candidate)) {
                $type = 'upload';
                $attribute['help'] = trans('mixtra.text_default_help_upload');
            ...
            if (in_array($field, $phone_candidate)) {
                ...
                $attribute['placeholder'] = trans('mixtra.text_default_help_number');
            }

            if (in_array($field, $email_candidate)) {
                $type = 'email';
                $validation[] = 'email|unique:'.$table;
                $attribute['placeholder'] = trans('mixtra.text_default_help_email');
            }

            if ($type == 'text' && in_array($field, $name_candidate)) {
                $attribute['placeholder'] = trans('mixtra.text_default_help_text');
                $validation = ['required', 'string', 'min:3', 'max:70'];
            }

            if ($type == 'text' && in_array($field, $url_candidate)) {
                $validation = ['required', 'url'];
                $attribute['placeholder'] = trans('mixtra.text_default_help_url');
            }
