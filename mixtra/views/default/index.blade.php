 		...
 		<div class="card">
            <div class="card-header">
            	<div class="row">
	            	<div class="col-sm-4 selected-action sm-action sm-bulk">
						...
						<div class="btn-group">
							...
							<div class="dropdown-menu">
								...
								<a class="dropdown-item small" data-name='delete' href="javascript:void(0)" title="{{trans('mixtra.action_delete_selected')}}" type='normal'><i class="fa fa-trash"></i> {{trans('mixtra.action_delete_selected')}} </a>
								@endif
								@if($button_selected)
									@foreach($button_selected as $button)
										<a class="dropdown-item small" href="javascript:void(0)" data-name='{{$button["name"]}}' title='{{$button["label"]}}' type="{{$button['type']}}"><i
		            ...
		            <div class="col-sm-8 text-right sm-action">
							...
							@if($button_filter)
			                    <a href="javascript:void(0)" id='btn_advanced_filter' data-url-parameter='{{$build_query}}'
			                       title="{{trans('mixtra.filter_dialog_title')}}" class="btn btn-secondary {{(Request::get('filter_column'))?'active':''}}">
			                        <i class="fa fa-filter"></i> <span class="sm-hide">{{trans("mixtra.button_filter")}}</span>
			                    </a>
			                @endif
		                </div>
		                <div class="btn-group">
	        ...
	        <div class="card-footer row" style="margin-top: -20px;">
	        	<div class="col-sm-8 sm-footer">{!! urldecode(str_replace("/?","?",$result->appends(Request::all())->render())) !!}</div>

				<div class="col-sm-4 mt-1 text-right sm-footer">
					<p>
						<?php
