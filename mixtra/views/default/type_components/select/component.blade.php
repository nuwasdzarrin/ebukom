<?php $default = ! empty($form['default']) ? $form['default'] : trans('mixtra.text_prefix_option')." ".$form['label'];?>
        ...
        <script type="text/javascript">
            $(function () {
                $('#{{$parent}}, input:radio[name={{$parent}}]').change(function () {
                    ...
                    if (fk_value != '') {
                        $current.html("<option value=''>{{trans('mixtra.text_loading')}} {{$form['label']}}");
...
@endif
    ...
    <div class="{{$col_width?:'col-sm-10'}}"> 
        <select class='form-control custom-select' id="{{$name}}" data-value='{{$value}}' {{$required}} {!!$placeholder!!} {{$readonly}} {{$disabled}} name="{{$name}}">
            <?php
            if (empty($form['cleardefault']) || $form['cleardefault'] != 'Y') {
            ?>
