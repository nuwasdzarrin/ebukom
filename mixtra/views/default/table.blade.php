...
<form id='form-table' method='post' action='{{MITBooster::mainpath("action-selected")}}'>
	...
	<!-- <div class="table table-responsive table-hover table-striped table-bordered"> -->
	    ...
	    <table id='table_dashboard' class="table table-hover table-striped table-bordered color-table {{ MITBooster::mySkin() }}-table">
	        <thead>
	            <tr>
		                ...
		                if (isset($sort_column[$field])) {
		                    switch ($sort_column[$field]['sorting']) {
		                        case 'asc':
		                            ...
		                            echo "<a href='$url' class='' title='Click to sort descending'>$colname &nbsp; <i class='fa fa-sort-desc'></i></a>";
		                            break;
		                        case 'desc':
		                            $url = MITBooster::urlFilterColumn($field, 'sorting', 'asc');
		                            echo "<a href='$url' class='' title='Click to sort ascending'>$colname &nbsp; <i class='fa fa-sort-asc'></i></a>";
		                            break;
		                        default:
		                            $url = MITBooster::urlFilterColumn($field, 'sorting', 'asc');
		                            echo "<a href='$url' class='' title='Click to sort ascending'>$colname &nbsp; <i class='fa fa-sort'></i></a>";
		                            break;
		                    }
		                } else {
		                    $url = MITBooster::urlFilterColumn($field, 'sorting', 'asc');
	        ...
	        @foreach($html_contents['html'] as $i=>$hc)
                ...
                <?php
                    $class = '';
                    $add_j = 0;
                    if($button_bulk_action) $add_j++;
                    if($show_numbering) $add_j++;
                    if ($columns[$j-$add_j]['hide']) {
                        $class = 'sm-hide';
                    }
                ?>
	        ...
	        <tfoot>
		        <tr>
		            ...
		            foreach ($columns as $col) {
		                ...
		                $class = $col['hide'] ? "sm-hide" : "";
		                echo "<th width='$width' class='$class'>$colname</th>";
		            }
		            ?>
 ...
 -->
                                            ...
                                            <div class='col-sm-6'>
                                                <div class='input-group {{ ($col["type_data"] == "time")?"bootstrap-timepicker":"" }}'>
                                                    ...
                                                    <input
                                                            ...
                                                            readonly placeholder='{{$col["label"]}} {{trans("mixtra.filter_from")}}'
                                            ...
                                            <div class='col-sm-6'>
                                                <div class='input-group {{ ($col["type_data"] == "time")?"bootstrap-timepicker":"" }}'>
                                                    ...
                                                    <input
                                                            ...
                                                            type='text'
                                                            class='filter-value-between form-control form-control-sm {{ (in_array($col["type_data"],["date","datetime","timestamp"]))?"datepicker":"timepicker" }}'
                                                            readonly placeholder='{{$col["label"]}} {{trans("mixtra.filter_to")}}'
                                                            name='filter_column[{{$col["field_with"]}}][value][]' value='<?php
                                                    $value = MITBooster::getValueFilter($col["field_with"]);
        ...
        <div class="modal fade" tabindex="-1" role="dialog" id='export-data'>
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                        <h4 class="modal-title"><i class='fa fa-download'></i> {{trans("mixtra.export_dialog_title")}}</h4>
