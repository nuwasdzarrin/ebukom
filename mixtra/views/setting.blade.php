        ...
        @foreach($set as $s)
            <script type="text/javascript">
                $(document).ready(function () {
                    $('#summernote_{{$s->name}}').summernote({
                        height: 300,
                        callbacks: {
                            onImageUpload: function (image) {
                                uploadImage{{$s->name}}(image[0]);
                            }
                        }
                    });

                    function uploadImage{{$s->name}}(image) {
                        ...
                        $.ajax({
                            ...
                            success: function (url) {
                                ...
                                $('#summernote_{{$s->name}}').summernote("insertNode", image[0]);
                            },
                            error: function (data) {
                                console.log(data);
                            }
...
//                        echo "<textarea name='$s->name' class='form-control form-control-sm wysiwyg'>$value</textarea>";
                        echo "<textarea id='summernote_".$s->name."' name='".$s->name."' class='form-control form-control-sm wysiwyg' rows='5'>$value</textarea>";
                        break;
                    case 'upload':
                    case 'upload_image':
                    ?>
