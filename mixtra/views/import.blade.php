...
@section('content')
    ...
    @if($button_show_data || $button_reload_data || $button_new_data || $button_delete_data || $index_button || $columns)
        <div id='box-actionmenu' class='box'>
            <div class='box-body'>
                @include("mitbooster::default.actionmenu")
            </div>
        </div>
    @endif
    ...
    <div class="col-12">
        ...
        <ul class="nav nav-tabs" role="tablist">
            <li class="nav-item"> 
            	<a class="nav-link"
            		...
            		onclick="if(confirm('Are you sure want to leave ?')) location.href='{{ MITBooster::mainpath("import-data") }}'">
            		<span class="hidden-sm-up"><i class="fa fa-download"></i></span> <span class="hidden-xs-down">Upload a File</span>
        		</a> 
        	</li>
            <li class="nav-item"> <a class="nav-link" href="#" role="tab"><span class="hidden-sm-up"><i class="fa fa-cogs"></i></span> <span class="hidden-xs-down">Adjustment</span></a> </li>
            <li class="nav-item"> <a class="nav-link active" href="#" role="tab"><span class="hidden-sm-up"><i class="fa fa-cloud-download-alt"></i></span> <span class="hidden-xs-down">Importing</span></a> </li>
        </ul>
                ...
                <script type="text/javascript">
                    $(function () {
                        ...
                        var int_prog = setInterval(function () {
                            ...
                            $.get("{{ MITBooster::mainpath('do-import-chunk?file='.Request::get('file')) }}", {resume: 1}, function (resp) {
                        ...
                        $.get("{{ MITBooster::mainpath('do-import-chunk').'?file='.Request::get('file') }}", function (resp) {
                            if (resp.status == true) {
        ...
        <div class="card-footer" id='upload-footer' style="display:none">
            <div class='text-right'>
                <a href='{{ MITBooster::mainpath("import-data") }}' class='btn btn-primary'><i class='fa fa-upload'></i> Upload Other File</a>
                <a href='{{ MITBooster::mainpath() }}' class='btn btn-success'>Finish</a>
            </div>
        </div><!-- /.box-footer-->
        ...
        <ul class="nav nav-tabs" role="tablist">
            ...
            <li class="nav-item"> <a class="nav-link active" href="#" role="tab"><span class="hidden-sm-up"><i class="fa fa-cogs"></i></span> <span class="hidden-xs-down">Adjustment</span></a> </li>
            <li class="nav-item"> <a class="nav-link" href="#" role="tab"><span class="hidden-sm-up"><i class="fa fa-cloud-download-alt"></i></span> <span class="hidden-xs-down">Importing</span></a> </li>
        </ul>
        ...
        if ($data_sub_module) {
            ...
            $action_path = MITBooster::mainpath();
        }
        ...
        <form method='post' id="form" enctype="multipart/form-data" action='{{$action}}'>
                ...
                <table class='table table-bordered' style="width:130%">
                    ...
                    <tr>
                        @foreach($table_columns as $k=>$column)
                            ...
                            <td data-no-column='{{$k}}'>
                                <select style='width:120px' class='form-control form-control-sm select_column' name='select_column[{{$k}}]'>
            ...
            <div class="card-footer">
                <div class='text-right'>
                    <a onclick="if(confirm('Are you sure want to leave ?')) location.href='{{ MITBooster::mainpath("import-data") }}'" href='javascript:;'
        ...
        <form method='post' id="form" enctype="multipart/form-data" action='{{$action}}'>
            ...
            <div class="card-body">
                ...
                <div class='alert alert-success'>
                    <h4>{{trans("mixtra.import_grettings")}}</h4>
                ...
                <div class='form-group'>
                    ...
                    <footer><small class='text-muted'>File type supported only : XLS, XLSX, CSV</small></footer>
                </div>
            </div><!-- /.box-body -->

            <div class="card-footer">
                <div class='text-right'>
                    <a href='{{ MITBooster::mainpath() }}' class='btn btn-default'>Cancel</a>
