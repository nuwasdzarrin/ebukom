            ...
            <form method='post' action='{{ (@$row->id)?route("PrivilegesControllerPostEditSave")."/$row->id":route("PrivilegesControllerPostAddSave") }}'>
                ...
                <div class="card-body">
                    ...
                    <div id='privileges_configuration' class='form-group'>
                        <label>{{trans('mixtra.privileges_configuration')}}</label>
                        @push('bottom')
                            <script>
                                $(function () {
                        ...
                        <table class='table table-striped table-hover table-bordered'>
                            <thead>
                            <tr class='active'>
                                <th width='3%'>{{trans('mixtra.privileges_module_list_no')}}</th>
                                <th width='60%'>{{trans('mixtra.privileges_module_list_mod_names')}}</th>
                                <th>&nbsp;</th>
                                <th>{{trans('mixtra.privileges_module_list_view')}}</th>
                                <th>{{trans('mixtra.privileges_module_list_create')}}</th>
                                <th>{{trans('mixtra.privileges_module_list_read')}}</th>
                                <th>{{trans('mixtra.privileges_module_list_update')}}</th>
                                <th>{{trans('mixtra.privileges_module_list_delete')}}</th>
                            </tr>
                            <tr class='info'>
                                <th>&nbsp;</th>
                                <th>&nbsp;</th>
                            ...
                            @foreach($moduls as $modul)
                                <?php
                                $roles = DB::table('mit_privileges_roles')->where('mit_modules_id', $modul->id)->where('mit_privileges_id', $row->id)->first();
                                ?>
                                <tr>
                                    <td><?php echo $no++;?></td>
                                    <td>{{$modul->name}}</td>
