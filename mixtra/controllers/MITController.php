...
class MITController extends Controller
{
    ...
    public function mitLoader()
    {
        ...
        $this->data['save_type'] = $this->save_type;
    ...
    public function getIndex()
    {
        ...
        if (Request::get('q')) {
            $result->where(function ($w) use ($columns_table, $request) {
                foreach ($columns_table as $col) {
                    ...
                    // dd($condition);
                    $w->orwhere($col['field_with'], "like", "%".$condition."%");
                }
            });
        }
        ...
        foreach ($data['result'] as $row) {
            ...
            if ($this->show_numbering) {
                $html_content[] = "<div class='text-center'>".$number."</div>";
                $number++;
            }
    ...
    public function getFindData()
    {
        ...
        if ($q || $id || $table1) {
            ...
            if (is_array($id)) {
                $rows->whereIn($table1.".".$table1PK, $id);
            } elseif($id) {
            ...
            if ($format) {
                ...
                if ($q) {
                    $rows->where($orderby_table.'.'.$orderby_column, 'likes', '%'.$q.'%');
                }
            ...
            if ($exp && count($exp)) {
                foreach ($exp as &$validationItem) {
                    if (substr($validationItem, 0, 6) == 'unique') {
                        ...
                        // $validationItem = 'unique:mit_materials,name';
                        // dd($validationItem);
                    }
                }
            } else {
    ...
    public function input_arr($ro)
    {
        ...
        if ($name) {
            if ($inputdata != '') {
                ...
                if (MIT::isColumnNULL($this->table, $name) && $ro['type'] != 'upload') {
                    return false;
                } else {
                    $this->arr[$name] = "";
                }
...
//                                 $relationship_table_pk => MITBooster::newId($ro['relationship_table']),
    ...
    public function getEdit($id)
    {
        ...
        $this->mitLoader();
        $row = DB::table($this->table)->where($this->primary_key, $id)->first();
        $next_data = DB::table($this->table)->where($this->primary_key, '>', $id)->orderBy($this->primary_key)->first();
        $next_id = $next_data ? $next_data->id : 0;
        ...
        return view('mitbooster::default.form', compact('id', 'row', 'page_menu', 'page_title', 'command', 'next_id'));
    }
